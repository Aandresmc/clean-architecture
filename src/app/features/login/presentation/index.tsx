...
import { Form, useForm } from "app/shared/components/useform";

import { FormComponent } from "./components";
import { TitleComponent } from "app/shared/components/title";
...
const LoginPage: React.FC<Props> = ({ authUseCase }: Props) => {
    ...
    const validate = (fieldValue: UserForm = values) => {
        ...
        if (fieldValue === values) {
            let formValid = Object.values(formErrors).every(errorField => errorField === '');
            return formValid
        }
    }
    ...
    return (
        <Form onSubmit={handleSubmit}>
            <Container>
                ...
                <FormComponent user={values} isLoading={isLoading} setValueField={handleInputChange} errors={errors} />
            </Container>
